...
/* 
	...
	◼ 视频端口 0
		分辨率
			最大输出分辨率：7680×4320@60Hz
			分量位深：10bit
		彩条
			水平模式
			垂直模式
		BCSH（10bit）
			亮度（8bit）、对比度（9bit）、饱和度（10bit）、色调调节（9bit）
		3D_LUT
			支持 9×9×9（12bit）LUT
		CSC（色彩空间转换）
			YUV 转 RGB：bt601-f /bt709-l /bt601-l /bt2020
			RGB 转 YUV：bt601-f /bt709-l /bt601-l /bt2020
		Gamma
		向下抖动（Dither down）
			Allegro
			FRC
		后级行缓冲模式（Post LB mode）
			4096 像素 × 4 模式
			2048 像素 × 8 模式
		显示模式
			逐行转隔行 / 隔行显示
	...
	◼ 视频端口 1
		分辨率
			最大输出分辨率：4096×4320@60Hz
		分量位深：10bit
		彩条
			水平模式
			垂直模式
		BCSH（10bit）
			亮度（6bit）、对比度（9bit）、饱和度（10bit）、色调调节（9bit）
		3D_LUT
			支持 9×9×9（12bit）LUT
		CSC
			YUV 转 RGB：bt601-f /bt709-l /bt601-l /bt2020
	...
	⚫ Cluster 0/1/2/3（集群通道）
		...
		数据格式
			...
			线性格式：RGBA8888 / RGB888 / RGB565
			8×8 块模式：YUV420（8/10bit）/ YUV422（8/10bit）/ YUV444（8/10bit）
		图像功能
			支持虚拟宽度（最大 8K 字）
			支持有效区域偏移
			支持显示偏移
			支持 Y 轴镜像 / X 轴镜像 / 旋转 90° / 旋转 270°
			若开启旋转 90° 或 270°，源图像最大高度为 2048
			通道交换：alpha / RB / UV 交换
			支持 YUV 裁剪
		集群行缓冲模式
			模式 0：1 × 4096 宽度图层模式
			模式 1：2 × 2048 宽度图层模式
			模式 2：1 × 2048 宽度 AFBCD 旋转图层模式
		支持高斯滤波
		缩放
			缩小
				支持缩放比例：1/4～1
		...
		CSC
			YUV 转 RGB：bt601-f /bt709-l /bt601-l /bt2020
			RGB 转 YUV：bt601-f /bt709-l /bt601-l /bt2020
	⚫ Esmart 0/1/2/3（智能图层通道）
		分辨率
			双 Esmart 合并：7680×4320
			单 Esmart：4096×4320
		数据格式
		...
		CSC
			...
			RGB 转 YUV：bt601-f /bt709-l /bt601-l /bt2020
	⚫ Overlay（叠加层）
		支持最多 8 层叠加：4 路 Cluster / 4 路 Esmart
		支持 RGB/YUV 域叠加
		支持图层混合
		支持逐像素 / 全局 Alpha
		支持 HDR10
